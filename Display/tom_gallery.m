function [out lookup]=tom_gallery(in,dims,spacings,border,background)
...
if nargin<2
    dims(1)=ceil(sqrt(size(in,3))); 
    dims(2)=ceil(sqrt(size(in,3))); 
end;
...
for i_m=1:m
    ...
    for i_n=1:n
        ...
        lookup(zz,:)=[round((start_n+start_n+dim_n-1)./2) round((start_m+start_m+dim_m-1)./2)];
        zz=zz+1;
        catch
        end;
