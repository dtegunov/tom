...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[]) {
...

    {
        ...
        if (mxIsComplexVolume) {
            if (mxType == mxSINGLE_CLASS) {
                ...
                size_t x, y, z;
                float *data_as_real;
                const float *data_real, *data_complex;
                ...
                data = data_as_real = (float *)mxGetData(mxData);
                data_real = (const float *)mxGetData(PRHS_VOLUME);
                data_complex = (const float *)mxGetImagData(PRHS_VOLUME);
                for (z=0; z<sizez; z++) {
                    for (y=0; y<sizey; y++) {
                        for (x=0; x<sizex; x++) {
                ...
                snprintf(s, __MAX_S__LENGTH__, "%s: Complex data for this type not supported (currently only for single).", mexFunctionName());
                mexErrMsgTxt(s);
            }
        }
    ...
    if (res != TOM_ERR_OK) {
        if (res ==TOM_ERR_WRITE_FILE) {
            ...
            snprintf(s, __MAX_S__LENGTH__, "%s: Wrong typeconversion: can not convert volume of type %d to type %d as in the em-file.", mexFunctionName(), getIOTypeFromMxClassID(mxType, mxIsComplexVolume), tom_io_em_get_iotype(&header));
            mexErrMsgTxt(s);
