/****************************************************************************//**
 * \file volume.hpp
 * \brief The header file for the class Wedge.
 * \author  Thomas Haller
 * \version 0.1
 * \date    22.11.2007
 *******************************************************************************/
#ifndef ___INCLUDE_CORE__VOLUME_HPP__
#define ___INCLUDE_CORE__VOLUME_HPP__
...
public:
    ...
    void write_to_em(const std::string &filename, const tom_io_em_header *header) const;
	void write_to_em(const std::string &filename, const tom_io_em_header *header, const uint32_t* first_voxel) const;
    void setValues(T val);
    void setValues(const Volume<T> &v);
    template <typename T2> void setValues(const Volume<T2> &v);

    template<typename T2> void shift_scale(T2 factor_shift, T2 factor_scale);

    bool is_equal_size(std::size_t s) const { return this->is_equal_size(s,s,s); }
    bool is_equal_size(std::size_t x, std::size_t y, std::size_t z) const { return this->getSizeX()==x && this->getSizeY()==y && this->getSizeZ()==z; }
    template<typename T2> bool is_equal_size(const tom::Volume<T2> &v2) const;

    double mean() const;
    double variance(bool use_sample_standard_deviation) const;
    double variance_mean_free(bool use_sample_standard_deviation) const;
    T min() const;
    T max() const;
    void stat(double &m, double &variance, bool use_sample_standard_deviation) const;
    void stat(double &m, double &variance, T &min, T &max, bool use_sample_standard_deviation) const;
	...
	bool equal_memory(const tom::Volume<T> &vol) const;
...
template<typename T> void read_from_em(tom::Volume<T> *&v, const std::string &filename, const uint32_t *subregion, const uint32_t *sampling, const uint32_t *binning, tom_io_em_header *header, void *(*fcn_malloc)(size_t size)=NULL, void (*fcn_free)(void *ptr)=NULL);
...
inline tom::Volume<T>::Volume(Volume<T2> &v, bool get_real_part) {
    ...
    // as for example int, where not std::complex<int> exists.
 ...
 *******************************************************************************/
template <typename T>
inline void (*tom::Volume<T>::get_free_function())(void *ptr) {
 ...
 *******************************************************************************/
template <typename T>
inline double tom::Volume<T>::mean() const {
    double mean;
    this->stat(&mean, NULL, NULL, NULL, false);
    return mean;
}
/****************************************************************************//**
 ...
 *******************************************************************************/
template <typename T>
inline double tom::Volume<T>::variance(bool use_sample_standard_deviation) const {
    double variance;
    this->stat(NULL, &variance, NULL, NULL, use_sample_standard_deviation);
    return variance;
}
 ...
 *******************************************************************************/
template <typename T>
inline T tom::Volume<T>::min() const {
    T min;
    this->stat(NULL, NULL, &min, NULL, false);
    return min;
}
 ...
 *******************************************************************************/
template <typename T>
inline T tom::Volume<T>::max() const {
    T max;
    this->stat(NULL, NULL, NULL, &max, false);
    return max;
}
 ...
 *******************************************************************************/
template <typename T>
inline void tom::Volume<T>::stat(double &m, double &variance, bool use_sample_standard_deviation) const {
 ...
 *******************************************************************************/
template <typename T>
inline void tom::Volume<T>::stat(double &m, double &variance, T &min, T &max, bool use_sample_standard_deviation) const {
    this->stat(&m, &variance, &min, &max, use_sample_standard_deviation);
}

/****************************************************************************//**
 * \brief Compute the minumum and the maximum of the volume.
 *******************************************************************************/
template<typename T>
inline void tom::Volume<T>::minmax(T &min, T &max) const {
    this->stat(NULL, NULL, &min, &max, false);
}
 ...
 *******************************************************************************/
template<typename T>
inline bool tom::Volume<T>::equal_memory(const tom::Volume<T> &vol) const{
