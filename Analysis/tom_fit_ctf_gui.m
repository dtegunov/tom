...
%tom_fit_ctf_gui graphical user interface for CTF fit.
%   
%
%
%PARAMETERS
...
% Last Modified by GUIDE v2.5 04-Mar-2010 16:51:56
...
if isempty(varargin)==0
    ...
    if tom_isemfile([pathname filename])==1
        ...
        disp(['cannot read as an EM-file: ' filename]);
    end;
end;
...
if header.Header.Size(1)<513
    in=tom_emreadc([pathname filename]);
else
...
set(handles.gui_intended_defocus,'String',handles.intended_defocus);
set(handles.gui_defocus_fixed,'String',handles.defocus_fixed);
disp(['Micrograph ' pathname filename ' reloaded']);
guidata(hObject, handles);
...
ctf.Fit=handles.Fit;
ctf.Search=Search;
flag_dynamic=get(handles.dynamic_defocus_search,'Value');
tom_fit_ctf_folder(handles.pathname,handles.file_ext,handles.pathname,ctf,'none',0,'',flag_dynamic);
...
target_path=handles.pathname;
if (strcmp(target_path(end),'/')==1)
   target_path=target_path(1:(end-1));
end;

target_path=[target_path '_corr/'];
...
if (strcmp(handles.pathname,target_path))
    Answ=questdlg('source path == target path  quit?');
...
flag_dynamic=get(handles.dynamic_defocus_search,'Value');
tom_fit_ctf_folder(handles.pathname,handles.file_ext,target_path,ctf,method,corr_cut_off_pix,'',flag_dynamic);
...
%    
    ...
    function handles=create_ps(handles)
        ...
        if handles.region(1)<handles.image_fullsize(1)
         ...
         v=get(handles.gui_generator,'Value');
            s=get(handles.gui_generator,'String');
            s=s(v);
            if strcmp(s,'tom_calc_periodogram')==1

        ps=tom_calc_periodogram_parallel(single(in.Value),handles.size(1),0,floor(handles.size(1)./16));
        handles.untreated_ps=fftshift(ps);
        ps=(log(fftshift(ps)));
            end;
            if strcmp(s,'tom_calc_periodogram (nolog)')==1
        ...
        function handles=correct_bg(handles)
            ps=handles.ps;
            v=get(handles.gui_correctbackground,'Value');
            %disp('Correct background: ');
            s=get(handles.gui_correctbackground,'String');
            s=s(v);
            if strcmp(s,'calc_decay')==1
            ...
            if strcmp(s,'rot_sym_Gauss')==1          
                % see Marin Van Heel et al.
                % Single particle electron cro-microscopy: towards atomic
                % resolution
                % Quarterly Reviews of Biophysics 33,4 (2000), pp 307-369
                % Page 321 and 322
                
                fs = size(ps,1);                
                rotational_symmetric_Gaussian = (1-single(mat2gray(fspecial('gaussian',fs,fs/handles.correctbackground_outer_radius)))).*...
                        single(mat2gray(fspecial('gaussian',fs,fs/handles.correctbackground_inner_radius)));               
                t=zeros(size(ps)); t(size(ps)./2+1)=1;
                handles.decay_image=tom_apply_weight_function(t,rotational_symmetric_Gaussian);
                handles.background_corrected_ps=tom_apply_weight_function(ps,rotational_symmetric_Gaussian);
            end
            if strcmp(s,'tom_filter(ps,128)')==1
                    handles.decay_image=tom_filter(ps,128);
            end
            if strcmp(s,'tom_filter(ps,16)')==1
                    handles.decay_image=tom_filter(ps,16);
            end
            if strcmp(s,'none')==1
                    handles.decay_image=0;
            end
            if strcmp(s,'rot_sym_Gauss')==1
            else
...
% --- Executes on button press in dynamic_defocus_search.
function dynamic_defocus_search_Callback(hObject, eventdata, handles)
% hObject    handle to dynamic_defocus_search (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of dynamic_defocus_search
