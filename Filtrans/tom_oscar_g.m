function [ccfmax, phi_max, psi_max, theta_max] = tom_oscar_g(vol,template,phi_start, phi_end, phi_incr, ...
...
gfor phi=phi_start:phi_incr:phi_end
    gfor psi=psi_start:psi_incr:psi_end
        gfor the=the_start:the_incr:the_end
            temp = gzeros(nvolx, nvoly, nvolz);
            temp(offsetx:offsetx+ntemplx-1,offsety:offsetx+ntemply-1,offsetz:offsetz+ntemplz-1)= ...
                gsingle(tom_rotate(template,[phi,psi,the]));
            gforce(temp);
