...
%--------------------------------------------------------------------------
% MARKER3D Free Tilt CM
%--------------------------------------------------------------------------
disp(' ');
disp('------------------------------------------------------------------');
disp('Welcome to Marker3d Free Tilt CM');
disp('------------------------------------------------------------------');
disp(' ');
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
if (a==0)
FreeTiltCMresults = fminsearch(@FreeTiltCMfun,StartValues,options);
end
if (a==1)
FreeTiltCMresults = lsqnonlin(@FreeTiltCMfun,StartValues,lb,ub,options);
end
%--------------------------------------------------------------------------
...
% Tilt axis azimuth
for i=1:v
alpha(i) = FreeTiltCMresults(3.*s+2.*v+i);
end
alphaMean = mean(alpha);

% 3d Marker coordinates
for j=1:s
    mx(j) = FreeTiltCMresults(j+0.*s);
    my(j) = FreeTiltCMresults(j+1.*s);
    mz(j) = FreeTiltCMresults(j+2.*s);
end
...
% Translations
for i=1:v
    tx(i) = FreeTiltCMresults(i+3.*s+0.*v);
    ty(i) = FreeTiltCMresults(i+3.*s+1.*v);
end
...
% MODEL FUNCTION
%--------------------------------------------------------------------------
function FreeTiltCMvalues = FreeTiltCMfun(x)
...
for j=1:s % Cost function
    ...
    FreeTiltCMvalues(zaehler) = MinimizeThis(1);
    zaehler = zaehler + 1;
    
    FreeTiltCMvalues(zaehler) = MinimizeThis(2);
    zaehler = zaehler + 1;
...
if (a==0)  % Simplex Search Method
   ...
   FreeTiltCMvaluesSum = 0;
    
    for k=1:(zaehler-1)
        FreeTiltCMvaluesSum = FreeTiltCMvaluesSum + (FreeTiltCMvalues(k))^2;
    end
    
    FreeTiltCMvalues = FreeTiltCMvaluesSum;

end

if (a==1)  % Conjugate gradients
    FreeTiltCMvalues = FreeTiltCMvalues;
end
%--------------------------------------------------------------------------
