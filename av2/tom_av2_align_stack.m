function [align2d new_ref]=tom_av2_align_stack(stack_path,ref_path,align2d,stack_alg_path,ref_alg_path,filter_param,paraell_param,iterations,demo,output_flag)
...
error_m=0;
correction_flag='pre Alignment';

shift_corr_flag=1;
h=tom_reademheader(stack_path);
size_stack=h.Header.Size;
h=tom_reademheader(ref_path);
if (h.Header.Size(3) > 1);
    correction_flag='pre Alignment';
else
    correction_flag='post Alignment';
end;
...
for iiii=1:max_num_iter
    ...
    if (num_of_tasks==1)
        ...
        for i=1:num_of_tasks
            createTask(j,@tom_av2_multi_ref_alignment,1,{stack_path,ref_path,filter_param,correction_flag,number_of_alg_iter,packages(i,1:2),i,demo});
        end;
        submit(j);
