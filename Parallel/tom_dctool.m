function varargout = tom_dctool(varargin)
%TOM_DCTOOL Graphical interface for status of distributed computing network
%
%   varargout = tom_dctool(varargin)
%
%TOM_DCTOOL Graphical interface for displaying the status of a distributed
%computing network
%
%      TOM_DCTOOL, by itself, creates a new TOM_DCTOOL or raises the existing
%      singleton.
%
%      H = TOM_DCTOOL returns the handle to a new TOM_DCTOOL or the handle to
...
%   ...
%
%   created by Tim Farajian 7/11/2005
%
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @tom_dctool_OpeningFcn, ...
    'gui_OutputFcn',  @tom_dctool_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% GUI Output function
function varargout = tom_dctool_OutputFcn(ignore, ignore2, handles)
...
function tom_dctool_OpeningFcn(hObject, ignore, handles)
% Choose default command line output for tom_dctool
...
fid = fopen('tom_dctool_help.txt');
str = fread(fid,inf,'*char').';
fclose(fid);
f = figure('menubar','none','NumberTitle','off','Name','About TOM_DCTOOL');
