function av3_scan_angles_global(refilename, motlfilename, particlefilename, index, ...
    ang_start, ang_end, angincr, mask,hipass,lowpass,wedgelist,iclass, ibin)
% AV3_SCAN_ANGLES_GLOBAL aligns 3D subtomograms to reference
%
%   av3_scan_angles_global(refilename, motlfilename, particlefilename, ...
%       index, ang_start, ang_end, angincr, mask,hipass,lowpass,wedgelist,iclass, ibin)
...
%   index                index - Index of reference AND motl
%   ang_start           angles [phi_start psi_start the_start]
%   ang_start           angles [phi_end psi_end the_end]
...
%   ibin                binning - if set > 0 binning is perfromed for
...
name = [refilename '_' num2str(index) '.em'];
ref = tom_emread(name);
...
name = [motlfilename '_' num2str(index) '.em'];
motl = tom_emread(name);
motl = motl.Value;
%indx = find (motl(1,:) > 0); meanv = mean(motl(1,indx));
%indx = find (motl(1,:) > threshold*meanv);
itomo_old = 0;
phi_start = ang_start(1);psi_start = ang_start(2);the_start = ang_start(3);
phi_end = ang_end(1);psi_end = ang_end(2);the_end = ang_end(3);
disp(['angular scan from phi = ' num2str(phi_start) ' psi = ' num2str(psi_start) ' the = ' num2str(the_start) ]);
disp(['  to phi = ' num2str(phi_end) ' psi = ' num2str(psi_end) ' the = ' num2str(the_end) ]);
for indpart = 1:size(motl,2)
    if ((motl(20,indpart) == 1) | (motl(20,indpart) == 2) | (motl(20,indpart) == iclass))
        ...
        [mn, mx, minv, std] = tom_dev(particle,'noinfo');
        particle = tom_limit((particle-mn)/std,-3,4,'z'); % throw away the gold
        ...
        for phi = phi_start:angincr:phi_end
            for the =  the_start:angincr:the_end
                for psi = psi_start:angincr:psi_end
        ...
        if size(average,1)>64
            if size(ccf)>90
                tom_dspcub(tom_bin(average,2));
            else
...
name = [motlfilename '_' num2str(index+1) '.em'];
...
name = [refilename '_' num2str(index+1) '.em'];
