function [shiftvec, angles, ccc, ccf_max, phivol, psivol, thevol] = av3_dock_nonorm(ref,particle,hipass,lowpass,angincr,startangles,endangles,flag)
% AV3_DOCK_nonorm aligns 3D reference (e.g. from X-ray) to particle (from EM)
%
%   [shiftvec, angles, ccc, ccf_max, phi_opt, psi_opt, the_opt] = av3_dock_nonorm(ref,particle,hipass,lowpass,angincr,startangles,endangles,flag)
%
...
if (nargin == 8)
    ...
    elseif (strmatch(flag,'phase-only','exact') == 1);
        ...
        fpart = ifftshift(tom_spheremask(fftshift(fpart),lowpass+1,2));
        fref = tom_fourier(ref);ii = find(abs(fref)< 10^(-6));
        fref = fref./(abs(fref));fref(ii)=0;
        fref = ifftshift(tom_spheremask(fftshift(fref),lowpass+1,2));
        ref = real(tom_ifourier(fref));
    end;
end;
...
energy_part = sum(sum(sum(abs(fpart).^2)));fpart = npixels*fpart/sqrt(energy_part);
icount = 0;
flag = 0;
...
for phi = phistart:angincr:phiend
    for psi = psistart:angincr:psiend
        for the = thestart:angincr:theend
            ...
            energy_ref = sum(sum(sum(abs(fref).^2)));fref = npixels*fref/sqrt(energy_ref);
