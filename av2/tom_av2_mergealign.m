...
%TOM_AV2_MERGEALIGN is a GUI for merging 2D particle alignment files ...
% and creating stacks from the alignment structure
%
%   varargout = tom_av2_mergealign(varargin)
%
%PARAMETERS
%
%  INPUT
...
if iscell(filename) | ischar(filename)
    for i = 1:size(filename,1)
        ...
        if strcmp(filename(i,:),'.') ~= 1 & strcmp(filename(1,:),'..') ~= 1
...
for i=1:size(string,1)
    ...
    try align2d = rmfield(s.align2d,'selectedtowrite');s.align2d = align2d;end;
    tmpalign = struct();
...
if ~isempty(outfile)
    ...
    %create stack
    if ~isempty(outstack)
        alignedflag = get(findobj('Tag','checkbox_alignmerge_aligned'),'Value');
        
        normflag = get(findobj('Tag','alignmerge_normed'),'Value');
        tom_av2_createstack(outfile, '', outstack, outfile, alignedflag, normflag);
    end
...
for i=1:size(align,2)
    if strmatch(align(end,i).class,newclasses,'exact')
    else
