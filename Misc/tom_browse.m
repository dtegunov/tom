...
if strcmp(get(handles.tom_browse_figure,'SelectionType'),'open')
    do_it(handles,filename, type);
        ...
        switch type
            ...
            case 'e2h5'; read_as_e2h5(handles,filename); return;
            case 'f3d'; read_as_f3d(handles,filename); return;
            ...
            case 'HPCL logfile';  read_as_hpcl_logfile(handles,filename); return;
            otherwise;
...
switch type
    ...
    case 'e2h5'; in=tom_eman2_read(filename);
...
if isequal(type,'Matlab file') || isequal(type,'C file') ...
        ...
        || isequal(type,'Header file') || isequal(type,'Python script') || isequal(type,'HPCL logfile')
    edit(filename);
end;
...
if isequal(type,'HPCL logfile')
    cmd=['tail -10 '  filename  ' &'];
    if isunix
        try
            [status,result] = unix(cmd);
        catch
...
switch type
    ...
    case 'spider'
        ...
        header=tmp.Header;
    case 'f3d'
        tmp=hdf5info(filename);
        dims=tmp.GroupHierarchy.Datasets.Dims;
        header.Size(1)=dims(1);
        header.Size(2)=dims(2);
        header.Size(3)=dims(3);
    case 'mrc'
...
switch type
    case 'em'
        head=tom_reademheader(filename);
        sz=head.Header.Size;
        if dim==2
            if binning_factor==0
                in=tom_emreadc2(filename);
            else
                in=tom_emreadc3(filename,[0,0,0,sz(1),sz(2),1],[1 1 1],[2^binning_factor 2^binning_factor 1]);
%                in=tom_emreadc2(filename,'binning',[binning_factor binning_factor 1]);
    ...
    case 'e2h5'
        in=tom_eman2_read(filename);
        in=tom_bin(in.Value,binning_factor);
    case 'f3d'
        in=f3d_read_volume(filename);
        in=imresize(in,1./(2.^binning_factor));
end;
        ...
        if findstr(action,'chimera') 
            if isunix
                switch type
                    ...
                    case 'f3d'
                        vol=f3d_read_volume(filename);
                        filename=[tempdir 'tom_tmp2chimera.em'];
                        tom_emwritec(filename,vol);
                        unix(['chimera --title ' filename ' tom_em:' filename ' &']);                                                
                    case 'e2h5'
                        unix(['chimera --title ' filename ' ' filename ' &']);
...
if size(X,3)>3
    handles.image=imshow(X(:,:,1:3),'Parent',handles.display);
else
...
set(handles.display,'YColor','white');
    ...
    function read_as_hpcl_logfile(handles,filename)
        %cmd=['tail -10 '  filename]
        if isunix
            try
                fid = fopen(filename, 'rt');
                status=fseek(fid,-500,'eof');
                for i=1:10
                    c(i,:) = fread(fid, 50, 'uint8=>char')';
                end;
                fclose(fid);
                handles.image=imshow(zeros(256,256),'Parent',handles.display);
                t0=text(20,20,'HPCL\_logfile','Parent',handles.display);
                set(t0,'Color','red');
                set(t0,'Fontsize',16);
                warning off;
                for i=1:10
                    t1=text(30,30+i.*20,c(i,:),'Parent',handles.display);
                    set(t1,'Color','red');
                end;
                warning on;
            catch
...
function read_as_e2h5(handles,filename)

t=tom_eman2_read(filename);
in.Header=t.Header;
t=t.Value;
tr=t(1:4:end,1:4:end);
m=mean2(tr);
...
function read_as_f3d(handles,filename)
in=hdf5info(filename);
dims=in.GroupHierarchy.Datasets.Dims;
if size(dims,2)>2
    t=squeeze(f3d_read_volume(filename,[dims(1) dims(2) 1],[0 0 dims(3)./2+1]));
    tr=t(1:4:end,1:4:end);
    m=mean2(tr);
    s=std2(double(tr));
    try
    ...
    t=hdf5read(filename,in.GroupHierarchy.Datasets.Name);    
    m=mean2(t);
    s=std2(double(t));
    try
        handles.image=imshow(t,'Parent',handles.display,'DisplayRange',[m-3.*s m+3.*s]);
