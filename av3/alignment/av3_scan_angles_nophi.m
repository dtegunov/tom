function av3_scan_angles_nophi(refilename, motlfilename, particlefilename, startindx, iterations,angincr, ...
    angiter,mask,hipass,lowpass,nfold,threshold,wedgelist,iclass, ibin, dispflag)
% AV3_SCANANGLES_nophi aligns 3D subtomograms to reference
%
...
for ind = startindx:startindx+iterations-1
    ...
    for indpart = 1:size(motl,2)
        if ((motl(20,indpart) == 1) | (motl(20,indpart) == 2) | (motl(20,indpart) == iclass))
            ...
            for phi = phi_old-angiter*0:angincr:phi_old+0*angincr% changed for omitting phi_loop
