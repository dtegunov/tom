...
%TOM_PARTICLES is a tool for interactive 3D particles picking 
%
%   varargout = tom_particles(varargin)
%
%PARAMETERS
%
%  INPUT
...
% Last Modified by GUIDE v2.5 02-Nov-2009 16:01:05
...
for lauf=1:128
    ...
    rand_pos(1)=abs(rand_pos(1)-8)+1;
    rand_pos(2)=abs(rand_pos(2)-8)+1;
%    disp([num2str(rand_pos(1)) ' ' num2str(rand_pos(2)) ' ' num2str(rand_pos(3)) ])
...
m=handles.output;
bg_color=get(handles.pickit,'BackgroundColor');

while get(hObject,'Value')==1
    set(m,'Pointer','crosshair');
    set(handles.pickit,'BackgroundColor',[0.8 0.2 0]);
    k = waitforbuttonpress;
    if k==0 && get(hObject,'Value')==1 %mouse button press
        point1 = get(gca,'CurrentPoint');    % button down detected
        pt = round(point1(1,1:2));
        if pt(1)<handles.Header.Header.Size(1) && pt(2)<handles.Header.Header.Size(2) && pt(1)>1 && pt(2)>1
            Rad = 10;
            x=pt(1);y=pt(2);
        ...
        set(handles.pickit,'BackgroundColor',bg_color);drawnow;
    end;
    guidata(hObject,handles);
end;
set(m,'Pointer','arrow');
set(handles.pickit,'BackgroundColor',bg_color);
guidata(hObject,handles);
...
if eventdata.VerticalScrollCount > 0 
         val=get(handles.XY_slider,'Value');
         val=val+1;
         if val>handles.Header.Header.Size(3)
             val=handles.Header.Header.Size(3);
         end;
         set(handles.XY_slider,'Value',val);
  ...
  XY_slider_Callback(handles.XY_slider, eventdata, handles);
