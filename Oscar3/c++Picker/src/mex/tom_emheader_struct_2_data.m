...
if (exist('data_type','var') && ~isempty(data_type) && ~(ischar(data_type)&&isempty(data_type)))
    if (~ischar(data_type))
        error('Datatype must be a string (int8, int16, int32, double, single, single_complex)');
    end;
    switch lower(data_type)
        ...
        case 'single_complex'
            data_type = 8;
        otherwise
            error('Unexpected datatype (only int8, int16, int32, double, single, single_complex allowed)');
    end;
else
    data_type = [];
    if (isfield(st, 'Value'))
