...
/*	       shell = (int)shell_float;
	       ...
	       phares_v = sqrt(amplitude1)+sqrt(amplitude2);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
   ...
   tom_comparec(mxGetData(VOL1),mxGetData(VOL2),mxGetData(VOLD),dims[0],mxGetScalar(NUM),mxGetScalar(START),mxGetScalar(STOP),mxGetData(OUT));
   return;
   }
