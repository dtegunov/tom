...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[]) {
    ...
    if ((res = tom_io_em_read_header(filename, "rb", &header, &f)) != TOM_ERR_OK) {
        if (res == TOM_ERR_OPEN_FILE) {
            snprintf(s, __MAX_S__LENGTH__, "%s: Could not open the file \"%s\"", mexFunctionName(), filename);
        } else {
            snprintf(s, __MAX_S__LENGTH__, "%s: Error reading the header (not a valid EM-file?) (%d)", mexFunctionName(), res);
        }
        mexErrMsgTxt(s);
    }
    ...
    if ((res=tom_io_calculate_sizes(header.dims, subregion, subregion_field, sampling, sampling_field, binning, binning_field, NULL, dims_l)) != TOM_ERR_OK) {
        ...
        if (res == TOM_ERR_SUBREGION_OUT) {
            ...
            snprintf(s, __MAX_S__LENGTH__, "%s: An unexpected error is returned when calculating the resulting size of the subregion. This might be a bug.", mexFunctionName());
        }
        mexErrMsgTxt(s);
    }


    iotype = tom_io_em_get_iotype(&header);
    restype = iotype;
...

    {
        ...
        mx_dims[1] = dims_l[1];
        mx_dims[2] = dims_l[2];
        if (restype == TOM_IO_TYPE_COMPLEX4) {
            mx_dims[2] *= 2;
        ...
        if ((res=tom_io_read_vol(f, header.dims, iotype, tom_io_em_is_swaped(&header), subregion_field, sampling_field, binning_field, mxGetData(mxData), restype)) != TOM_ERR_OK) {
            ...
            switch (res) {
                case TOM_ERR_NO_COMPLEX_BINNING:
                    snprintf(s, __MAX_S__LENGTH__, "%s: Binning of complex data-type is not implemented. (%d).", mexFunctionName(), res);
                    break;
                default:
                    snprintf(s, __MAX_S__LENGTH__, "%s: Error reading the data from file (%d).", mexFunctionName(), res);
                    break;
            }
            mexErrMsgTxt(s);
        }
