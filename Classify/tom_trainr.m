function [net,tr,Ac,El]=tom_trainr(net,Pd,Tl,Ai,Q,TS,VV,TV)
...
% $Revision: 1.1.6.2 $  $Date: 2005/12/22 18:21:02 $
...
for epoch=0:epochs
 ...
 if length(stop), break; end
  ...
  for q=randperm(Q)
    ...
    X = getx(net);
    perf = feval(performFcn,Elq(net.hint.targetInd,:),Ac(net.hint.targetInd,:),X,net.performParam);

    % Gradient
    if (needGradient)
      gE = cell(net.numOutputs,TS);
      gE(net.hint.targetInd,:) = feval(performFcn,'dy',Elq(net.hint.targetInd,:),Ac(net.hint.targetInd,:),X,perf,net.performParam);
      ...
      for i=1:numLayers
        ...
        for j=find(net.inputConnect(i,:))
          ...
          if length(learnFcn)
           ...
           net.userdata.IW_hist{i,j,epoch+1}=net.IW{i,j};
          %net.userdata.hist{epoch+1}=net;
          end
        end
  ...
  perf = feval(performFcn,El,Ac,X,performParam);
  end
end
