function ret = tom_av3_oscar_feed2(volumefile, templatefile, Outputfile, psffile, maskfile, logfile, fftsize, nr_procs, angle_start, angular_incr, angle_end, scriptflag)
...
%molmatch_path='/fs/home/korinek/oscar_sun/bin/oscar';
...
if scriptflag == 1
    ...
    %[status result]=unix(['ssh berlin "cd /fs/sandy02/lv05/pool/bmsan/rockel/pool-b_baumei/EMANcombi_6/REINER/SEGS/RUN_3/OUTPUT8SEGS && mpirun -ssi rpi tcp -np ' num2str(nr_procs) ' ' molmatch_path ' ' volumefile ' ' templatefile ' ' Outputfile  ' ' angles ' ' psffile ' ' maskfile ' ' num2str(fftsize) ' | tee ' logfile '"'],'-echo');
    
    [status result]=unix(['ssh berlin "cd /fs/sally07/lv01/pool/pool-nickell/thermosome/data_gutsche/Processing/ATP/auto_refinements/all_part_start_cyl_restart && mpirun -ssi rpi tcp -np ' num2str(nr_procs) ' ' molmatch_path ' ' volumefile ' ' templatefile ' ' Outputfile  ' ' angles ' ' psffile ' ' maskfile ' ' num2str(fftsize) ' | tee ' logfile '"'],'-echo');
