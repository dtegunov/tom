...
%TOM_FIT_CTF performs CTF fitting of a decay corrected power spectrum.
%
%   [Fit]=tom_fit_ctf(in,EM,Search)
%
%PARAMETERS
%
%  INPUT
%   in                  decay corrected, masked power spectrum
%   EM                  structure with parameters of the electron
%                       microscope, such as Voltage, Cs etc.
%   Search              structure with search paramters, such as
%                       defocus, astigmatism etc.
%  
% Structures in detail:
% EM = 
% 
%     Objectpixelsize: 1.6000e-10
%             Voltage: 200000
%                  Cs: 0.0020
%                  Cc: 0.0022
% 
...
% Search = 
% 
%                      Dz_search: [1x21 double]
%                Dz_delta_search: 0
%                   Phi_0_search: 0
%      amplitude_contrast_search: 0.0700
%                   decay_search: 0
%      decay_part_coh_ill_search: 0
%     decay_energy_spread_search: 0.8000
%              mask_inner_radius: 9
%              mask_outer_radius: 127
%           mask_inner_radius_bg: 8
%           mask_outer_radius_bg: 128
%                        ps_size: [256 256]
% 
% 
% Fit = 
...
%                Dz_delta_det: 0
%                   Phi_0_det: 0
%      amplitude_contrast_det: 0.0700
%                   decay_det: 0
%      decay_part_coh_ill_det: 0
%     decay_energy_spread_det: 0.8000
%                    corr_all: [21x1 double]
%             corr_statistics: [1x1 struct]
%           mask_inner_radius: 9
%           mask_outer_radius: 127
%                          EM: [1x1 struct]
%  OUTPUT
%   Fit                 structure with determined fitting parameters.
%
%  This function is parallelized, use 'matlabpool open' to start a parallel
%  session.
%
%EXAMPLE
% see tom_fit_ctf_gui and use button: 'Copy Fit to Workspace' for
% variables, structure info.
%
%REFERENCES
%
%SEE ALSO
...
% add some new quality paramters to the Fit structure:
[s d]=size(corr_all);
corr_all_reshape=reshape(Fit.corr_all,[s.*d 1]);
[me, ma, mi, st, va] = tom_dev(corr_all_reshape,'noinfo');
Fit.corr_statistics.max=ma;
Fit.corr_statistics.min=mi;
Fit.corr_statistics.mean=me;
Fit.corr_statistics.std=st;
Fit.corr_statistics.var=va;
Fit.corr_statistics.z_score=(ma-me)./st;
