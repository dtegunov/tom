function [nr,pos_out,val_out,angle_out,shift_out]=tom_av2_index_search(index_stack,lookup,part,align_flag,mask_cc,demo_flag)
...
for i=1:100000
    ...
    if (std2(index_stack(:,:,sind_left))==0  || std2(index_stack(:,:,sind_left))==0)
        break;
    end;
    ...
    if (strcmp(align_flag,'pre Alignment'))
        [t_angle_out t_shift_out cc]=tom_av2_align(idx_left,part,ones(size(part)),tom_cart2polar(ones(size(part))),mask_cc,'',2,0);
    ...
    %right branch
    if (strcmp(align_flag,'pre Alignment'))
        [t_angle_out t_shift_out cc]=tom_av2_align(idx_right,part,ones(size(part)),tom_cart2polar(ones(size(part))),mask_cc,'',2,0);
