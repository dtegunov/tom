function av3_phialig(iref, refilename, motlfilename, particlefilename, startindx, mask, ...
    hipass, lowpass, nfold, threshold, wedgelist,iclass, ibin, dispflag)  
%
...
%
%  PARAMETERS
%   iref                Index of reference particle. This particle is
%                       chosen as the FIRST reference. Make sure that the 
%                       filenumber exists...
...
%
%   09/18/03 FF
%last change 04/29/05 FF - dispflag added
...
for ii = 1:size(motl,2)
    ...
    if ( ((motl(20,indpart) == 1) | (motl(20,indpart) == 2) | (motl(20,indpart) == iclass)) && (indpart~=indref) )
        ...
        if (strcmp(dispflag,'nodisp')~= 1 )
            figure(2);plot(phiccf);title('polar CCF');%drawnow;
        end;
