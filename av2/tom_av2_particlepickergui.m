...
%TOM_AV2_PARTICLEPICKERGUI is a  GUI for picking particles from 2D EM images
%
%   varargout = tom_av2_particlepickergui(varargin)
%
%PARAMETERS
%
%  INPUT
...
try
   ...
   disp('cache demon not working');
end;

pickList_in_flag=0;

if (length(varargin)>1)
    set(handles.bandpass_high,'String',varargin{2});
    set(handles.bandpass_switch,'Value',1);
end;
...
if size(varargin,1) == 0
    ...
    if (isstruct(varargin{1}))
        pl_tmp=varargin{1};
        storage_av2_particlepicker.path=fileparts(pl_tmp(1,1).filename);
        pickList_in_flag=1;
    end;   
...
storage_av2_particlepicker.polycell = {};

if (pickList_in_flag==0)
    [storage_av2_particlepicker.dircell, storage_av2_particlepicker.flagcell] = get_dircontents(storage_av2_particlepicker.path,{},{});
else
     storage_av2_particlepicker.dircell=[];
     storage_av2_particlepicker.flagcel=[];
    for i=1:size(pl_tmp,2)
        [a b c]=fileparts(pl_tmp(1,i).filename);
        all_names_tmp{i}=[b c];
    end;
    names_u=unique(all_names_tmp);
    
    for i=1:length(names_u)
        storage_av2_particlepicker.dircell{i}=names_u{i};
        storage_av2_particlepicker.flagcell{i}=1;
    end;
    set(findobj('Tag','fileslider'),'Max',size(storage_av2_particlepicker.dircell,2),'SliderStep',[1./size(storage_av2_particlepicker.dircell,2) 1./size(storage_av2_particlepicker.dircell,2)]);
...
storage_av2_particlepicker.imagenumber_high = storage_av2_particlepicker.imagenumber;

storage_av2_particlepicker.reference_image = struct();


if (pickList_in_flag==1)
...
%exece resample
storage_av2_particlepicker.resampleval= str2double(get(handles.input_resample,'String'));
...
%input_resample_Callback(hObject, eventdata, handles);
...
show_high=get(handles.show_pl_high,'Value');
if (show_high==1)
    render_image_high(position,1);
end;
drawnow;
auto_save();
...
if isempty(resampleval) | resampleval < 1 | resampleval > 20
    errordlg('Valid resampling range: 1-20');
        ...
        for k = 1:size(storage_av2_particlepicker.points(i).points,2)
            ...
            if strmatch(storage_av2_particlepicker.align(1,j).class,classname,'exact');
                ...
                if x <= radius*2 | y <= radius*2 | x > imagesize(1)-radius*2-1 | y > imagesize(2)-radius*2-1
                    ...
                    part_box = tom_emreadc2([storage_av2_particlepicker.align(1,j).filename],'subregion',[lowx lowy 1],[highx-lowx highy-lowy 0]);
                    ...
                    particle = tom_emreadc2(storage_av2_particlepicker.align(1,j).filename,'subregion',[x-2*radius y-2*radius 1],[4*radius-1 4*radius-1 0]);
...
if ischar(filename)
    ...
    val = get(findobj('Tag','menu_classes'),'Value');
    classname = contents{val};
    im = tom_emreadc2([pathname '/' filename]);
    storage_av2_particlepicker.reference_image.(classname) = tom_taper(im.Value,[im.Header.Size(1).*2 im.Header.Size(1).*2]);
    render_avg();
    render_filtered_avg();
end
...
%%  Save Reference                                                     %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function button_particlepicker_saveref_Callback(hObject, eventdata, handles)

global storage_av2_particlepicker;
[filename, pathname] = uiputfile('*.em', 'Save reference as');
if ischar(filename)
    contents = get(findobj('Tag','menu_classes'),'String');
    val = get(findobj('Tag','menu_classes'),'Value');
    classname = contents{val};
    im = storage_av2_particlepicker.reference_image.(classname);
    imsize = size(im);
    im = im(imsize(1)./2+1-imsize(1)./4:imsize(1)./2+imsize(1)./4,imsize(2)./2+1-imsize(2)./4:imsize(2)./2+imsize(2)./4);
    tom_emwrite([pathname '/' filename],im);
end
...
if ischar(filename)
   ...
   particlepicker.maskcell = storage_av2_particlepicker.polycell;
...
if ischar(filename)
    ...
    %old code buggy if dircell of filefilter differs from org dircell
   ...
   %loop over org dircell and set all files 2 bad
   for ii=1:length(storage_av2_particlepicker.flagcell)
        storage_av2_particlepicker.flagcell{ii}=0;
   end;
   
   %loop over new filefilter and grab corresponding names (new filefilter 2 dircell)
   zz=0;
   for ii=1:length(s.particlepicker.filelist)
       idx_dir=find(ismember(storage_av2_particlepicker.dircell,s.particlepicker.filelist{ii}));
       if (isempty(idx_dir)==0) 
            storage_av2_particlepicker.flagcell{idx_dir} = s.particlepicker.filefilter{ii};
            zz=zz+1;
       end;
   end;
   
   disp([num2str(zz) ' images in filefilter which correspond to ' num2str(length(storage_av2_particlepicker.flagcell)) ' images in selected directory']);
   
    try
        storage_av2_particlepicker.polycell = s.particlepicker.maskcell;
    catch
        storage_av2_particlepicker.polycell = {};
    end;
    render_image(storage_av2_particlepicker.imagenumber);
...
%%  Draw polygon                                                       %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function button_particklepicker_drawpoly_Callback(hObject, eventdata, handles)

global storage_av2_particlepicker;
tmpobj = findobj('Tag','image');
axes(tmpobj);
[BW,xi,yi]= tom_roipoly();

if size(storage_av2_particlepicker.polycell,2) >= storage_av2_particlepicker.imagenumber
    if (isempty(storage_av2_particlepicker.polycell{storage_av2_particlepicker.imagenumber})==0)
        storage_av2_particlepicker.polycell{storage_av2_particlepicker.imagenumber} = {storage_av2_particlepicker.polycell{storage_av2_particlepicker.imagenumber}{1:end} [xi.*storage_av2_particlepicker.resampleval,yi.*storage_av2_particlepicker.resampleval]};
    else
        storage_av2_particlepicker.polycell{storage_av2_particlepicker.imagenumber} = {[xi.*storage_av2_particlepicker.resampleval,yi.*storage_av2_particlepicker.resampleval]};
    end;
else
    storage_av2_particlepicker.polycell{storage_av2_particlepicker.imagenumber} = {[xi.*storage_av2_particlepicker.resampleval,yi.*storage_av2_particlepicker.resampleval]};
end
...
if storage_av2_particlepicker.flagcell{number} == 0
    ...
    if storage_av2_particlepicker.imagenumber ~= number || forcereload == 1
       try
           ...
           disp(Me.message);
           error(['Cannot read: ' storage_av2_particlepicker.path '/' storage_av2_particlepicker.dircell{number} ]);
       end;
       ...
       try
           [storage_av2_particlepicker.DataScale(1) storage_av2_particlepicker.DataScale(2)] = calculate_histogram(storage_av2_particlepicker.image(1:200:end,1:200:end));
       catch
           [storage_av2_particlepicker.DataScale(1) storage_av2_particlepicker.DataScale(2)] = calculate_histogram(storage_av2_particlepicker.image);
       end;
       try
           tom_HT_readahead({[storage_av2_particlepicker.path '/' storage_av2_particlepicker.dircell{number+1}]});
       catch
           disp('warning cache not working');
       end;
    ...
    if (get(findobj('Tag','bandpass_switch'),'Value') == 1)
        ...
        im = tom_filter(storage_av2_particlepicker.image, str2num(get(findobj('Tag','bandpass_high'),'String')), 'quadr','real');
    ...
    if (get(findobj('Tag','background_switch'),'Value') == 1)
        bg_val=str2double(get(findobj('Tag','background_cer'),'String'));
        sz_orgI=size(im);
        im=im- imresize(tom_cut_out(tom_filter(imresize(im,[2048 2048]+[bg_val.*1.3 bg_val.*1.3]),bg_val),'center',[2048 2048]),sz_orgI);
        im=tom_filter(im,3,'quadr','real');
        try
            [storage_av2_particlepicker.DataScale(1) storage_av2_particlepicker.DataScale(2)] = calculate_histogram(im(1:200:end,1:200:end));
        catch
            [storage_av2_particlepicker.DataScale(1) storage_av2_particlepicker.DataScale(2)] = calculate_histogram(im);
        end;
        DataScale = storage_av2_particlepicker.DataScale;
    end;
    
    %generate polygon mask
    if size(storage_av2_particlepicker.polycell,2) >= number && ~isempty(storage_av2_particlepicker.polycell{number})
        mask_st.Apply=1;
        mask_st.Value.size_x=size(im,1);
        mask_st.Value.size_y=size(im,2);
        mask_st.Polygons = storage_av2_particlepicker.polycell{number};
        mask_st.Value.binning = log2(storage_av2_particlepicker.resampleval);
...
function render_image_high(number, forcereload)
...
%skip images marked as bad
if storage_av2_particlepicker.flagcell{number} == 0
    if number >= storage_av2_particlepicker.imagenumber_high
        if number > size(storage_av2_particlepicker.flagcell)
            msgbox('End of image stack reached.');
        else   
            render_image_high(number+1);
        end
    else
        if number == 1
            msgbox('Beginning of image stack reached.');
    ...
    if storage_av2_particlepicker.imagenumber_high ~= number || forcereload == 1
       im = tom_emreadc([storage_av2_particlepicker.path_high '/' storage_av2_particlepicker.dircell{number}],'resample',[storage_av2_particlepicker.resampleval storage_av2_particlepicker.resampleval 1]);
       
       im.Value = single(im.Value);
       storage_av2_particlepicker.image_high = im.Value;
       storage_av2_particlepicker.header_high = im.Header;
       try
           [storage_av2_particlepicker.DataScale_high(1) storage_av2_particlepicker.DataScale_high(2)] = calculate_histogram(storage_av2_particlepicker.image_high(1:200:end,1:200:end),0);
       catch
           [storage_av2_particlepicker.DataScale_high(1) storage_av2_particlepicker.DataScale_high(2)] = calculate_histogram(storage_av2_particlepicker.image_high);
       end;
       try
           tom_HT_readahead({[storage_av2_particlepicker.path '/' storage_av2_particlepicker.dircell{number+1}]});
       catch
    ...
    if (get(findobj('Tag','bandpass_switch'),'Value') == 1)
        %im = tom_bandpass(storage_av2_particlepicker.image,str2num(get(findobj('Tag','bandpass_low'),'String')), str2num(get(findobj('Tag','bandpass_high'),'String')));
        im = tom_filter(storage_av2_particlepicker.image_high, str2num(get(findobj('Tag','bandpass_high'),'String')), 'quadr','real');
         DataScale = storage_av2_particlepicker.DataScale_high;
        %DataScale = single(storage_av2_particlepicker.DataScale) ./ (size(im,1) .* size(im,2));
        if str2num(get(findobj('Tag','bandpass_low'),'String')) > 0
           % DataScale = DataScale - mean(mean(storage_av2_particlepicker.image)) ./ (size(im,1) .* size(im,2));
        end
...
%     set(findobj('Tag','fileslider'),'Value',number);
    ...
    storage_av2_particlepicker.imagenumber_high = number;
    
    set(0,'CurrentFigure',storage_av2_particlepicker.h_pl_h);
    imhandle = imagesc(im',DataScale); axis image; colormap gray; 
    title([storage_av2_particlepicker.path_high '/' storage_av2_particlepicker.dircell{number}]);
    t=get(imhandle,'Parent'); set(t,'YTickLabel',[]); set(t,'XTickLabel',[]);
    scrsz = get(0,'ScreenSize');
    
    if (scrsz(3)>3500)
        set(storage_av2_particlepicker.h_pl_h,'position',[1925 24 1529 1097]);
    end;
...
%%  calculate histogram                                                %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [min,max] = calculate_histogram(image,disp_on)
...
pause(0.001);drawnow; pause(0.001);
...
if ( strcmp(button,'extend') == true)
    %position = round(get(hObject,'Value'));
    h=findobj('Tag','fileslider');
    position=round(get(h,'Value'))+1;
    set(h,'Value',position);
...
if strcmp(button,'alt') == true
    x=x1.*storage_av2_particlepicker.resampleval;
    y=y1.*storage_av2_particlepicker.resampleval;
    points = manage_align('getpointsincurrentframe');
    
   zz=1;
   pointcoords=zeros(length(points),2); 
   for i = points
        pointcoords(zz,:) = [storage_av2_particlepicker.align(1,i).position.x storage_av2_particlepicker.align(1,i).position.y];
        zz=zz+1;
    end
...
if get(findobj('Tag','show_switch'),'Value') == 1 || get(findobj('Tag','alignment_switch'),'Value') == 1
...
%     end
    ...
    if x1 <= radius*2 | y1 <= radius*2 | x1 > imagesize(1)-radius*2-1 | y1 > imagesize(2)-radius*2-1
        ...
        %cut out particle, this will give a non quadratic matrix
        part_box = tom_emreadc2([storage_av2_particlepicker.path '/' storage_av2_particlepicker.dircell{storage_av2_particlepicker.imagenumber}],'subregion',[lowx lowy 1],[highx-lowx highy-lowy 0]);
        part_box = single(part_box.Value);
        ...
        %normal cutout mode
        part_box = tom_emreadc2([storage_av2_particlepicker.path '/' storage_av2_particlepicker.dircell{storage_av2_particlepicker.imagenumber}],'subregion',[x1-2*radius y1-2*radius 1],[4*radius-1 4*radius-1 0]);
        part_box = single(part_box.Value);
    end
...
function h = drawmark_high(x,y,Color,Tag,Label)
...
elseif strcmp(varargin{1},'getpointsincurrentframe_high')
    
    name_l=storage_av2_particlepicker.dircell{storage_av2_particlepicker.imagenumber};
    zz=0;
    for i=1:size(storage_av2_particlepicker.pl_high,2)
        [p name_h ext]=fileparts(storage_av2_particlepicker.pl_high(1,i).filename);
        if (strcmp([name_h ext],name_l))
            zz=zz+1;
            out(zz)=i;
        end;
    end;
...
elseif strcmp(varargin{1},'rebuildindex')
    ...
    if (length(storage_av2_particlepicker.align(1,:))~=0 )
        fname_old=storage_av2_particlepicker.align(1,1).filename;
       %  newidx=storage_av2_particlepicker.points; %danger fb!!!!!!!!!!!!!!!!!!!!1
        for i=1:size(storage_av2_particlepicker.align,2)
            
            if (strcmp(storage_av2_particlepicker.align(1,i).filename,fname_old)==0 ||i==1)
                
                
%                 if (isfield(storage_av2_particlepicker.align,'fileidx') && isempty(storage_av2_particlepicker.align(1,i).fileidx)==0 )
%                     fileidx=storage_av2_particlepicker.align(1,i).fileidx;
%                 else
                   % disp(['using strfind' num2str(i)]);
                    [pathstr, name, ext] = fileparts(storage_av2_particlepicker.align(1,i).filename);
                    fileidx = strmatch([name ext],storage_av2_particlepicker.dircell);    
                    storage_av2_particlepicker.align(1,i).fileidx=fileidx;
               % end;
                
                
                fname_old=storage_av2_particlepicker.align(1,i).filename; 
                if (isempty(fileidx))
                    continue;
                end
        ...
        storage_av2_particlepicker.points = newidx;
         ...
         storage_av2_particlepicker.points=[];
    end;
...
function render_points_high()

global storage_av2_particlepicker;

counter = 0;

delete(findobj('-regexp','Tag','xphigh_*'));

try
    for i=manage_align('getpointsincurrentframe_high')
        counter = counter+1;
        if get(findobj('Tag','display_particlenumbers'),'Value') == 1
            label = num2str(counter);
        else
        ...
        drawmark_high(storage_av2_particlepicker.pl_high(1,i).position.x/storage_av2_particlepicker.resampleval, storage_av2_particlepicker.pl_high(1,i).position.y/storage_av2_particlepicker.resampleval,storage_av2_particlepicker.align(1,i).color,strcat('xphigh',storage_av2_particlepicker.align(1,i).label),label);
    end
catch ME
    disp(ME.message);
end
    ...
    for k = 1:size(storage_av2_particlepicker.points(i).points,2)
        ...
        if strmatch(storage_av2_particlepicker.align(1,j).class,classes,'exact');
            ...
            if x <= radius*2 | y <= radius*2 | x > imagesize(1)-radius*2-1 | y > imagesize(2)-radius*2-1
                ...
                if ((lowx < imagesize(1))  & (lowy < imagesize(2)) & (highx-lowx > 0) & (highy-lowy > 0) ) 
                    part_box = tom_emreadc2([storage_av2_particlepicker.align(1,j).filename],'subregion',[lowx lowy 1],[highx-lowx highy-lowy 0]);
                else
                    part_box.Value=zeros(2.*radius,2.*radius,'single');  
                end;
                part_box = single(part_box.Value);
...
% --- Executes on button press in show_switch.
function show_switch_Callback(hObject, eventdata, handles)
% hObject    handle to show_switch (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of show_switch


% --- Executes on mouse motion over figure - except title and menu.
function particlepickergui_WindowButtonMotionFcn(hObject, eventdata, handles)
% hObject    handle to particlepickergui (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
function particlepickergui_WindowScrollWheelFcn(hObject, eventdata, handles)
% hObject    handle to particlepickergui (see GCBO)
...
% --- Executes on button press in htl_mo_up.
function htl_mo_up_Callback(hObject, eventdata, handles)
% hObject    handle to htl_mo_up (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global storage_av2_particlepicker;
pos=manage_align('getpointsincurrentframe');
move=str2num(get(handles.htl_shift_pix,'String'));

for i=pos
    storage_av2_particlepicker.align(1,i).position.x=storage_av2_particlepicker.align(1,i).position.x;
    storage_av2_particlepicker.align(1,i).position.y=storage_av2_particlepicker.align(1,i).position.y-move;
end;

render_points()

% --- Executes on button press in htl_mo_do.
function htl_mo_do_Callback(hObject, eventdata, handles)
% hObject    handle to htl_mo_do (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global storage_av2_particlepicker;
pos=manage_align('getpointsincurrentframe');
...
% --- Executes on button press in htl_mo_ri.
function htl_mo_ri_Callback(hObject, eventdata, handles)
% hObject    handle to htl_mo_ri (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global storage_av2_particlepicker;
pos=manage_align('getpointsincurrentframe');
...
for i=pos
    storage_av2_particlepicker.align(1,i).position.x=storage_av2_particlepicker.align(1,i).position.x+move;
    storage_av2_particlepicker.align(1,i).position.y=storage_av2_particlepicker.align(1,i).position.y;
end;

render_points()

% --- Executes on button press in htl_mo_le.
function htl_mo_le_Callback(hObject, eventdata, handles)
% hObject    handle to htl_mo_le (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global storage_av2_particlepicker;
pos=manage_align('getpointsincurrentframe');
...
% --- Executes on button press in br_pl_hi.
function br_pl_hi_Callback(hObject, eventdata, handles)
% hObject    handle to br_pl_hi (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
storage_av2_particlepicker.pl_high=align2d;
[a b c]=fileparts(align2d(1,1).filename);
storage_av2_particlepicker.path_high=a;

% --- Executes on button press in show_pl_high.
function show_pl_high_Callback(hObject, eventdata, handles)
% hObject    handle to show_pl_high (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of show_pl_high

val=get(handles.show_pl_high,'Value');

global storage_av2_particlepicker;

h_pl_h=findobj('Tag','pl_high_extra');

if (val==1)
    if (isempty(h_pl_h))
        h_pl_h=figure;
        set(h_pl_h,'Tag','pl_high_extra');
    end;
else
    if (isempty(h_pl_h)==0)
        close(h_pl_h);
...
storage_av2_particlepicker.h_pl_h=h_pl_h;


position=round(get(handles.fileslider,'Value'));
...
function htl_shift_pix_Callback(hObject, eventdata, handles)
% hObject    handle to htl_shift_pix (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of htl_shift_pix as text
%        str2double(get(hObject,'String')) returns contents of htl_shift_pix as a double


% --- Executes during object creation, after setting all properties.
function htl_shift_pix_CreateFcn(hObject, eventdata, handles)
% hObject    handle to htl_shift_pix (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
% --- Executes on button press in background_switch.
function background_switch_Callback(hObject, eventdata, handles)
% hObject    handle to background_switch (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of background_switch
global storage_av2_particlepicker;
render_image(storage_av2_particlepicker.imagenumber,1);

function background_cer_Callback(hObject, eventdata, handles)
% hObject    handle to background_cer (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of background_cer as text
%        str2double(get(hObject,'String')) returns contents of background_cer as a double
global storage_av2_particlepicker;
render_image(storage_av2_particlepicker.imagenumber,1);

% --- Executes during object creation, after setting all properties.
function background_cer_CreateFcn(hObject, eventdata, handles)
% hObject    handle to background_cer (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
