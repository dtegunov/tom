function av3_trans_rot_alig_nophi(refilename, motlfilename, particlefilename, ...
    startindx, iterations,angincr, angiter,mask,hipass,lowpass,nfold,...
    threshold,wedgelist,iclass, ibin, dispflag)
% AV3_TRANS_ROT_ALIG_NOPHI aligns 3D subtomograms to reference
%
%   av3_trans_rot_alig_nophi(refilename, motlfilename, particlefilename, ...
%      startindx, iterations,angincr, angiter,mask,hipass,lowpass,nfold,...
%      threshold,wedgelist,iclass, ibin, dispflag)
%
...
%   orienations. 
%   A clever sampling on the unit sphere is done for psi and theta: rings
%   around (theta_old, psi_old) are drawn.
%   PHI is not scanned in this version.
...
%
%  SEE ALSO
%   AV3_TRANS-ROT-ALIG, AV3_PHIALIG, AV3_CREATEMOTL, molmatch.exe, 
%       omnimatch.exe
%
%   09/18/03 FF
...
for ind = startindx:startindx+iterations-1
    ...
    for indpart = 1:size(motl,2)
        if ((motl(20,indpart) == 1) | (motl(20,indpart) == 2) | (motl(20,indpart) == iclass))
            ...
            for phi = phi_old-0*angincr:angincr:phi_old+0*angincr %changed FF
